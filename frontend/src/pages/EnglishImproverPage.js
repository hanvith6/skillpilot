...
import { Languages, Copy, Sparkles, Zap, Check } from 'lucide-react';
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API_URL}/api/generate/english`,
        { text, emergent_mode: emergentMode },
        { headers: { Authorization: `Bearer ${token}` } }
      );

      setResult(response.data.data);
      updateCredits(user.credits - response.data.credits_used);
      toast.success(`Text improved! ${response.data.credits_used} credits used`);
    } catch (error) {
